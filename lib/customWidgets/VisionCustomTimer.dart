...
import 'VisionCustomTimerRemainingTime.dart';
...
class VisionCustomTimer extends StatefulWidget {
  VisionCustomTimer(
      {Key? key,
      required this.from,
      required this.to,
      ...
      this.onChangeState,
      this.onChangeStateAnimation = const AnimatedSwitcher(
  ...
  ///
  /// If null, this widget will create its own [VisionCustomTimerController] and dispose it when the widget is disposed.
  final VisionCustomTimerController? controller;
...
class _CustomTimerState extends State<VisionCustomTimer> {
  Widget? _returnWidget;
  Timer? _timer;
  late Duration _duration;
  late VisionCustomTimerController _controller;

  @override
  void initState() {
    _controller = widget.controller ?? new VisionCustomTimerController();
  ...
  void _startCountup() {
    _timer = Timer.periodic(widget.interval, (Timer timer) {
      if (_duration.inMilliseconds < widget.to.inMilliseconds) {
        // Countup finished.
        // Fluttertoast.showToast(msg: 'TimeUp', toastLength: Toast.LENGTH_SHORT, timeInSecForIosWeb: 1);
        _onTimerFinished();
      } else {
        // Fluttertoast.showToast(msg: 'Restart', toastLength: Toast.LENGTH_SHORT, timeInSecForIosWeb: 1);
